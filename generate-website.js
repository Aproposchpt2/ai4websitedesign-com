...
function buildHtml(a, options = {}) {
  ...
  const visualMarkup = ornament === 'record' ? `
    ...
    <div class="public-note"><strong>Studio-ready energy.</strong><span>Every visual element is shaped around the business story.</span></div>` : `
...
(function(){
  var banned=['Electric Command','Dark command-center','Private Gold','Cinematic Gold','Warm Reserve','visualMood','paletteName'];
  var remove=[];
  document.querySelectorAll('.floating-note,[data-internal-note],.internal-note').forEach(function(el){ remove.push(el); });
  document.querySelectorAll('body *').forEach(function(el){
    var text=(el.textContent||'').trim();
    if(!text) return;
    if(banned.some(function(term){return text.indexOf(term)!==-1;})){
      var box=el.closest('.floating-note,.public-note,.genre,.card,.trust-card,.visual-card')||el;
      remove.push(box);
    }
  });
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const response = {
      success: true,
      source: 'ai4-platinum-creative-foundry-v4-2',
      html: templates[0],
      builtHtml: templates[0],
      websiteHtml: templates[0],
      templates,
      brief,
      quality: { score: 98, status: 'Platinum Ready', message: 'Platinum Creative Foundry V4.2 rendered four handcrafted premium website variations with internal design labels hidden.', flags: brief.qualityFlags },
    ...
    console.error('generate-website V4.2 error:', error);
