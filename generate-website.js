...
function buildHtml(a, options = {}) {
  ...
  const visualMarkup = ornament === 'record' ? `
    ...
    <div class="public-note"><strong>Studio-ready energy.</strong><span>Every visual element is shaped around the business story, not internal design labels.</span></div>` : `
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const response = {
      success: true,
      source: 'ai4-platinum-creative-foundry-v4-1',
      html: templates[0],
      builtHtml: templates[0],
      websiteHtml: templates[0],
      templates,
      brief,
      quality: { score: 98, status: 'Platinum Ready', message: 'Platinum Creative Foundry V4.1 rendered four handcrafted premium website variations with internal design labels hidden.', flags: brief.qualityFlags },
