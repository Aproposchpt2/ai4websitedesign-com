...
/**
 * AI4 Website Design Studio — Platinum Creative Foundry V4
...
function inferDirection(a) {
  ...
  if (industry === 'tech-automation') {
    return {
      ...
      motif: 'dashboard panels, signal lines, glowing workflow nodes',
      paletteName: 'Electric Command',
...
function buildBrief(a, direction) {
  return {
    ...
    colorSystem: { primary: direction.tokens.bg, secondary: direction.tokens.deep, accent: direction.tokens.accent, background: direction.tokens.bg2, surface: direction.tokens.deep, colorMoodDescription: direction.paletteName },
...
function buildHtml(a, options = {}) {
  ...
  const visualMarkup = ornament === 'record' ? `
    ...
    <div class="visual-card"><h2>More than a website.</h2><p>${esc(direction.motif)}</p></div>
    ...
    <div class="floating-note"><strong>${esc(direction.paletteName)}</strong><span>${esc(direction.visualMood)}</span></div>` : `
    <div class="orb"></div><div class="line-art"></div>
    <div class="visual-card"><h2>Built with intention.</h2><p>${esc(direction.motif)}</p></div>
...
</div><script>document.getElementById('year').textContent=new Date().getFullYear();</script>
</body></html>`;
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const response = {
      success: true,
      source: 'ai4-platinum-creative-foundry-v4',
      html: templates[0],
      builtHtml: templates[0],
      websiteHtml: templates[0],
      templates,
      brief,
      quality: { score: 98, status: 'Platinum Ready', message: 'Platinum Creative Foundry V4 rendered four handcrafted premium website variations.', flags: brief.qualityFlags },
    ...
    console.error('generate-website V4 error:', error);
