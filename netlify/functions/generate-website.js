...
<section id="services"><div class="wrap"><div class="section-head"><div class="kicker">What We Deliver</div><h2 class="section-title">A polished experience built around what makes you different.</h2><p class="section-lead">AI4 turns simple business facts into elevated positioning, clear sections, and a premium visitor path designed to build trust fast.</p></div><div class="cards">${services.map(([x,y],i)=>`<article class="card"><div class="icon">${['✦','◆','✓'][i]||'✦'}</div><h3>${esc(x)}</h3><p>${esc(y)}</p></article>`).join('')}</div></div></section>
...
<section><div class="wrap quote-band"><h2>${direction.industry === 'music-studio' ? 'Your message deserves a sound that matches it.' : 'Your business deserves a first impression that matches its value.'}</h2><p>${esc(a.differentiators || 'A premium website should not feel generic. It should feel like your business has been translated into a clear, memorable, high-trust experience.')}</p><a class="btn primary" href="#contact">${cta}</a></div></section>
...
async function enrichAnswers(a) {
  ...
  const prompt = `You are the AI4 Website Design Studio 5-Karat Content Architect.

Your job is not to describe a business. Your job is to create FASCINATION.

You are the talent agent for this business. You are going to make them look like a star.

THE 5-KARAT MANDATE:
Every line you write must create at least one of: Curiosity, Trust, Desire, Authority, Exclusivity, Momentum.
If it creates none of those — rewrite it.
...
- Generic. Vague. Forgettable.

BUSINESS INTAKE (these are CLUES, not copy — transform them):
...
YOUR MISSION:
Infer their motivation, identity, credibility, desired perception, and opportunity.
Then write copy that makes THEM look like the premium choice in their market.
Write like a Hollywood talent agent promoting their biggest client.
The business name must be the star. Everything else is supporting cast.

QUALITY GATE — before finalizing each line, ask:
1. Would a professional copywriter be embarrassed to claim authorship? If yes, rewrite.
2. Could another business use this by swapping the name? If yes, rewrite.
3. Does this create desire or momentum? If no, rewrite.

Return ONLY a valid JSON object:
{
  "businessTagline": "4-7 words that define their market position — powerful, specific, memorable. This goes LARGE on the page.",
  "heroHeadline": "The most powerful statement about what this business IS — not what they do. 5-8 words. Create desire or authority.",
  "heroSubheadline": "The tension, the promise, or the revelation. 8-14 words. Make them lean in.",
  "heroDescription": "2-3 sentences (60-80 words) that speak to their ideal customer's deepest motivation. Promotional, specific, urgent. Reference the business name.",
  "service1Title": "Service card title — 3-5 words, benefit-forward, specific to this business",
  "service1Desc": "40-60 words — what this service actually does for the customer. Specific outcomes. No fluff.",
  "service2Title": "Service card title — 3-5 words",
  "service2Desc": "40-60 words — specific, benefit-driven, outcome-focused",
  "service3Title": "Service card title — 3-5 words",
  "service3Desc": "40-60 words — specific, compelling, closes with why it matters",
  "proof1Label": "1-3 word trust signal label — specific to this business",
  "proof1Desc": "One sentence proof point — concrete, credible, specific",
  "proof2Label": "1-3 word trust signal label",
  "proof2Desc": "One sentence proof point",
  "proof3Label": "1-3 word trust signal label",
  "proof3Desc": "One sentence proof point",
  "featureTitle": "Signature section headline — 6-10 words, bold claim about this business",
  "featureCopy": "70-90 words — the emotional core of why this business is different. This is the monologue. Make it memorable. Reference the business name. Make the reader feel something.",
  "ctaTagline": "The closing statement — 10-15 words. Create urgency or exclusivity. Make them act."
}

The customer receiving this website should feel: 'This was built for ME.'`;
