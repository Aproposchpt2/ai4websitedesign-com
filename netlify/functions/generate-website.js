...
/**
 * AI4 Website Design Studio — Platinum Creative Foundry V4.3
...
function buildHtml(a, options = {}) {
  ...
  const visualMarkup = ornament === 'record' ? `
    ...
    <div class="visual-card"><h2>More than a website.</h2><p>A premium visual experience shaped around the business story, audience, and action path.</p></div>
    ...
    <div class="visual-card"><h2>Built with intention.</h2><p>A premium first impression shaped around clarity, trust, and action.</p></div>
...
(function(){
  ...
  var remove=[];
  document.querySelectorAll('.floating-note,.public-note,[data-internal-note],.internal-note,.template-feature-card,.design-meta-card').forEach(function(el){ remove.push(el); });
  document.querySelectorAll('body *').forEach(function(el){
    var text=(el.textContent||'').trim();
    if(!text) return;
    if(banned.some(function(term){return text.indexOf(term)!==-1;})){
      var box=el.closest('.floating-note,.public-note,.template-feature-card,.design-meta-card,.genre,.card,.trust-card,.visual-card')||el;
      remove.push(box);
    }
  });
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const response = {
      success: true,
      source: 'ai4-platinum-creative-foundry-v4-3-no-internal-feature-card',
      html: templates[0],
      builtHtml: templates[0],
      websiteHtml: templates[0],
      templates,
      brief,
      quality: { score: 98, status: 'Platinum Ready', message: 'Platinum Creative Foundry V4.3.2 rendered four handcrafted premium website variations with internal design labels hidden.', flags: brief.qualityFlags },
    ...
    console.error('generate-website V4.3 error:', error);
